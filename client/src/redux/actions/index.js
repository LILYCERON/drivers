...
export function inOrder(value) {
    return async function (dispatch) {
        ...
        if (value === "az" || value === "za") {
            ...
            const orderDate = response.data.filter((driver) => driver.createdInDb === false)
            console.log(orderDate)
            return dispatch({
                type: GET_DRIVER_BY_SOURCE,
                payload: orderDate,
            })
        } else if (value === "db") {
            const orderDate = response.data.filter((driver) => driver.createdInDb === true)
            console.log(orderDate)
            return dispatch({
                type: GET_DRIVER_BY_SOURCE,
                payload: orderDate,
